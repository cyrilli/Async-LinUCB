...
class LocalClient:
    def __init__(self, dimension_g, dimension_l, lambda_, delta_, NoiseScale):
        ...
        self.alpha_t_g = self.NoiseScale * np.sqrt(
            self.d_g * np.log(1 + (self.numObs_local) / (self.d_g * self.lambda_)) + 2 * np.log(1 / self.delta_)) + np.sqrt(
            self.lambda_)
        self.alpha_t_l = self.NoiseScale * np.sqrt(
            self.d_l * np.log(1 + (self.numObs_local) / (self.d_l * self.lambda_)) + 2 * np.log(
    ...
    def localUpdate(self, articlePicked_FeatureVector, click, inner_iters=100):
        ...
        self.alpha_t_g = self.NoiseScale * np.sqrt(
            ...
            self.lambda_)
        self.alpha_t_l = self.NoiseScale * np.sqrt(
            self.d_l * np.log(1 + (self.numObs_local) / (self.d_l * self.lambda_)) + 2 * np.log(1 / self.delta_)) + np.sqrt(
            self.lambda_)
