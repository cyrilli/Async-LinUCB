...
class LocalClient:
	...
	def localUpdate(self, arm_featureVector):
		...
		self.b_local += arm_featureVector * (self.theta.T.dot(arm_featureVector) + np.random.randn()*self.sigma)
		self.arm_selection_local = 1

		self.A_uploadbuffer += self.matrix_dot(arm_featureVector)
		self.b_uploadbuffer += arm_featureVector * (self.theta.T.dot(arm_featureVector) + np.random.randn()*self.sigma)
...
class LinGapE:
	...
	def run(self, articles):
		...
		while(self.B > self.epsilon):
			...
			# assume have 100 users(clients)
			currentclientID = random.randint(1, 100)
		...
		return self.samplecomplexity, self.totalCommCost
